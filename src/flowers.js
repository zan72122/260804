...
export class FlowerSystem {
  constructor(scene) {
    ...
    for (const type of FLOWER_TYPE_LIST) {
      ...
      const im = new THREE.InstancedMesh(def.petalGeo(), mat, CAPACITY[type]);
      im.count = 0;
      im.castShadow = true;
    ...
    this.centerMesh = new THREE.InstancedMesh(cGeo, cMat, CENTER_CAP);
    ...
    this.calyxMesh = new THREE.InstancedMesh(kGeo, kMat, CALYX_CAP);
