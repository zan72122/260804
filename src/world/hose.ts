...
export class Hose {
  ...
  private buildNozzle(): void {
    ...
    const handleGeo = new THREE.TorusGeometry(0.34, 0.075, 8, 16, Math.PI);
  ...
  private buildCoupling(): void {
    ...
    const g1 = new THREE.TorusGeometry(1.3, 0.14, 8, 26);
    const ring = new THREE.Mesh(g1, mat);
    ring.rotation.x = -Math.PI / 2;
    this.coupling.add(ring);
    const g2 = new THREE.TorusGeometry(0.85, 0.08, 8, 22);
  ...
  update(dt: number, waterHeight: (x: number, z: number) => number, pumping: number): void {
    ...
    this.nozzle.position.set(nx, wy + 0.12 - suck + Math.sin(this.time * 2.1) * 0.03, nz);
    this.nozzle.rotation.z = Math.sin(this.time * 1.4) * 0.05;
    this.mouth.set(nx, wy - 0.34, nz);
    ...
    p[2].set(
      ...
      wy + 1.15 + Math.sin(this.time * 1.6) * 0.06,
