// ドームケーキ「ひみつのドームケーキ」🍫 — 既存レシピロジックの移植
// フェーズ: MIX → POUR → BAKE → LAYER → DECORATE → DOME → REVEAL → DONE
import * as THREE from '../../vendor/three.module.js';
import {
  createBowl, createSpoon, createStream, createPan, createSponge,
  createCreamSwirl, createPipingBag, createDecoration, createChocoDome,
  createDrips, createMeltPool, createPitcher, createDripCrown,
  createConfetti, createSparkles, createSteam, createIngredient,
} from '../cake.js';
import { tween, ease, clamp, lerp } from '../util.js';

export const meta = { id: 'dome', title: 'ひみつのドームケーキ', emoji: '🍫' };

export function createRecipe(ctx) {
  const { scene, stand, STAND_TOP, env, pointOnPlane, camPhase, pointer } = ctx;
  const { setHint, setProgress, showBanner, setFinger } = ctx.ui;
  const { sfx, setPourSound, setStirSound } = ctx.audio;
  ...
  // BAKE中はキャンバスをタップした瞬間に焼き開始（実イベントで即応、フレーム待ちなし）
  ...
  function enterPhase(p) {
    ...
    if (p === 'MIX') {
      ...
      doneBtnEl.classList.remove('show');
  ...
  // クリーム色選択
  for (const btn of paletteEl.querySelectorAll('button')) {
      ...
      switch (state.phase) {
        ...
        case 'DONE':
          state.doneTime += dt;
          {
            ...
            camPhase([Math.sin(a) * r, top + 0.22, Math.cos(a) * r], [0, top, 0]);
          }
          break;
      }
