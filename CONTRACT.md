...
- 角度はラジアン。スクリーンは canvas 標準（y 下向き）。ローカル角 θ の見た目の位置は
  `screen = view.center + R * view.scale * [cos(θ + state.rotation), sin(θ + state.rotation)]`。
- 画面下方向 = `+π/2`。`NCFG.ANCHOR_ANGLE = Math.PI/2`（次に切るスロットをここへ回す）。
...
- 描画内容: 和紙風背景 → 黒い菓子皿(艶ハイライト) → 練り切り本体(未カット部はやわらかマット球) → **切られた花びら**(lift に応じ立ち上がる。テーマの base→tip グラデ、縁に切り込みの影) → 中心(しべ: squish で締まり、完成で黄色いしべ粒) → `state.nextHint` があれば**次に切る位置ガイド**(パルスする✂マーク+きらめき。文字禁止)。
